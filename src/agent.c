...
int agent_set_remote_description(juice_agent_t *agent, const char *sdp) {
	...
	ice_description_t *remote = (ice_description_t *)malloc(sizeof(ice_description_t));
	if (!remote) {
		JLOG_FATAL("Memory allocation for remote description failed");
		conn_unlock(agent);
		return -1;
	}
...
int agent_relay_send(juice_agent_t *agent, agent_stun_entry_t *entry, const addr_record_t *dst,
	...
	int ret = agent_direct_send(agent, &entry->record, buffer, size, ds);
	free(buffer);
	return ret;
}
...
int agent_channel_send(juice_agent_t *agent, agent_stun_entry_t *entry, const addr_record_t *record,
	...
	int ret = agent_direct_send(agent, &entry->record, buffer, len, ds);
	free(buffer);
	return ret;
}
	...
	*next_timestamp = now + 6000000;
	...
	for (int i = 0; i < agent->entries_count; ++i) {
		...
		if (entry->state == AGENT_STUN_ENTRY_STATE_PENDING) {
			...
			if (entry->retransmissions >= 0) {
				...
				default:
					JLOG_DEBUG("end agent_bookkeeping entries_count: %d", agent->entries_count);
	...
	if (selected_pair) {
		...
		if (agent->selected_pair != selected_pair) {
			JLOG_DEBUG("%s", selected_pair->nominated ? "New selected and nominated pair"
			                                    	  : "New selected pair");
...
int agent_send_stun_binding(juice_agent_t *agent, agent_stun_entry_t *entry, stun_class_t msg_class,
	...
	char* buffer = (char*)malloc(BUFFER_SIZE);
	if (!buffer) {
		JLOG_ERROR("Failed to allocate buffer for STUN message");
		return -1;
	}
